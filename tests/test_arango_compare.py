    ...
    def test_compare_databases(self, mock_print_and_write):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            calls = [
                ...
                call("\n================================================================================", mock.ANY),
                call("\nFeature                                         DB1                  DB2", mock.ANY),
                call("\n--------------------------------------------------------------------------------", mock.ANY),
                call(f"\nTotal collections                                {summary1['total_collections']}                   {summary2['total_collections']}", mock.ANY),
                call(f"\nTotal documents                             {summary1['total_documents']}              {summary2['total_documents']}", mock.ANY),
                call(f"\nTotal indexes                                    {summary1['total_indexes']}                   {summary2['total_indexes']}", mock.ANY),
                call(f"\nTotal graphs                                      {summary1['total_graphs']}                    {summary2['total_graphs']}", mock.ANY),
                call(f"\nTotal analyzers                                  {summary1['total_analyzers']}                   {summary2['total_analyzers']}", mock.ANY),
                call(f"\nTotal views                                       {summary1.get('total_views', 0)}                    {summary2.get('total_views', 0)}", mock.ANY)
            ]
